...
DWA 作为传统局部规划算法，无需训练即可部署。其控制量直接由速度空间搜索得到，优点是响应快、结构清晰，但由于决策完全依赖当前时刻的局部代价，遇到障碍物密集或目标方向变化较快的场景时，容易表现出路径抖动、转向保守以及绕行效率不高等问题。\cite{lai2023enhanced}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/bad_transSAC/bad_transSAC_ASL_train_combined.png}
