...
通过上述关系，可以将电机角度 $\phi_1$、$\phi_4$ 映射到足端位置 $P(x_p, z_p)$，实现对足端位置的控制。公式\ref{eq:l0_phi0} 中的 $L_0$ 和 $\phi_0$ 即为后续虚拟模型控制中使用的虚拟腿长和摆角,描述了足端力/力矩 $\gamma$ 与关节电机力矩 $\tau$ 之间的映射关系，$J^T$ 是雅可比矩阵的转置。
...
其中 $J$ 是雅可比矩阵，描述了足端力/力矩 $\gamma$ 与关节电机力矩 $\tau$ 之间的映射关系，$J^T$ 是雅可比矩阵的转置。通过上述关系，可以将电机角度 $\phi_1$、$\phi_4$ 映射到足端位置 $P(x_p, z_p)$，实现对足端位置的控制。公式\ref{eq:l0_phi0} 中的 $L_0$ 和 $\phi_0$ 即为后续虚拟模型控制中使用的虚拟腿长和摆角。
