...
# 0) 符号
# ---------------------------
R_w, R_l = sp.symbols('R_w R_l', positive=True)
m_w, m_l, m_b, g = sp.symbols('m_w m_l m_b g')
I_w, I_b, I_z = sp.symbols('I_w I_b I_z')
# 左/右腿的几何分解：l_l = l_bl + l_wl,  l_r = l_br + l_wr
l_bl, l_br, l_wl, l_wr, l_c = sp.symbols('l_bl l_br l_wl l_wr l_c')
l_l, l_r = sp.symbols('l_l l_r')

# 轮角速度/加速度、腿角速度/加速度、机体
ddtheta_wl, ddtheta_wr = sp.symbols('ddtheta_wl ddtheta_wr')
ddtheta_ll, ddtheta_lr = sp.symbols('ddtheta_ll ddtheta_lr')
theta_ll, theta_lr, theta_b = sp.symbols('theta_ll theta_lr theta_b')  # 小角线性
ddphi = sp.symbols('ddphi')  # 偏航角加速度

# 轮/腿/机体力、力矩
f_l, f_r = sp.symbols('f_l f_r')                      # 轮-地切向力
F_wsl, F_bsl, F_wsr, F_bsr = sp.symbols('F_wsl F_bsl F_wsr F_bsr')  # 水平支撑/传递
F_whl, F_bhl, F_whr, F_bhr = sp.symbols('F_whl F_bhl F_whr F_bhr')  # 竖直支撑/传递
T_lwl, T_lwr, T_bll, T_blr = sp.symbols('T_lwl T_lwr T_bll T_blr')  # 轮/腿扭矩

# 线速度/加速度（来自 2.x 运动学）
dds, dds_b, dds_ll, dds_lr = sp.symbols('dds dds_b dds_ll dds_lr')
ddh_b, ddh_ll, ddh_lr = sp.symbols('ddh_b ddh_ll ddh_lr')

# ---------------------------
# 1) 原始 2.x 运动学（线性化）
# ---------------------------
# (2.9) + (2.13)
eq_dds   = sp.Eq(dds,   R_w/2*(ddtheta_wl + ddtheta_wr))
eq_ddsb  = sp.Eq(dds_b, R_w/2*(ddtheta_wl + ddtheta_wr) + (l_bl+l_wl)/2*ddtheta_ll + (l_br+l_wr)/2*ddtheta_lr)

# (2.18)-(2.19) & (2.22)-(2.23) 线性化（去掉 θ*dθ^2 等二阶项）
eq_ddsll = sp.Eq(dds_ll, R_w*ddtheta_wl + l_wl*ddtheta_ll)
eq_ddslr = sp.Eq(dds_lr, R_w*ddtheta_wr + l_wr*ddtheta_lr)
eq_ddhll = sp.Eq(ddh_ll, ddh_b + theta_ll*ddtheta_ll)   # 小角线性
eq_ddhlr = sp.Eq(ddh_lr, ddh_b + theta_lr*ddtheta_lr)   # 小角线性

# 偏航（2.11，线性化）
eq_ddphi = sp.Eq(ddphi, R_w/(2*R_l)*(-ddtheta_wl + ddtheta_wr) - (l_bl+l_wl)/(2*R_l)*ddtheta_ll + (l_br+l_wr)/(2*R_l)*ddtheta_lr)

# ---------------------------
# 2) 原始 3.x 动力学
# ---------------------------
# 轮的切向力与转动方程 (3.1)-(3.2)+(3.3)-(3.4)
eq_wl_lin = sp.Eq(m_w*R_w*ddtheta_wl, f_l - F_wsl)
eq_wr_lin = sp.Eq(m_w*R_w*ddtheta_wr, f_r - F_wsr)
eq_Twl    = sp.Eq(I_w*ddtheta_wl, T_lwl - f_l*R_w)
eq_Twr    = sp.Eq(I_w*ddtheta_wr, T_lwr - f_r*R_w)

# 腿的水平/竖直传递 (3.3)-(3.5)
eq_legL_h = sp.Eq(m_l*dds_ll, F_wsl - F_bsl)
eq_legR_h = sp.Eq(m_l*dds_lr, F_wsr - F_bsr)
eq_legL_v = sp.Eq(m_l*ddh_ll, F_whl - F_bhl - m_l*g)
eq_legR_v = sp.Eq(m_l*ddh_lr, F_whr - F_bhr - m_l*g)

# 机体平动 (3.6)-(3.7)
eq_body_s = sp.Eq(m_b*dds_b, F_bsl + F_bsr)
eq_body_h = sp.Eq(m_b*ddh_b, F_bhl + F_bhr - m_b*g)

# 机体俯仰 (3.8)
eq_pitch  = sp.Eq(I_b*sp.symbols('ddtheta_b'),
                  -(T_bll+T_blr) + (F_bsl+F_bsr)*l_c + (F_bhl+F_bhr)*l_c*theta_b)

# 偏航 (3.9)
eq_yaw = sp.Eq(I_z*ddphi, (-f_l + f_r)*R_l)

# ---------------------------
# 3) 消元：先用轮的方程把 f_l, f_r, F_wsl, F_wsr 表成 ddθ 和 T
# ---------------------------
sol_wl = sp.solve([eq_wl_lin, eq_Twl], (f_l, F_wsl), dict=True)[0]
sol_wr = sp.solve([eq_wr_lin, eq_Twr], (f_r, F_wsr), dict=True)[0]
# f_l = (T_lwl - I_w ddθ_wl)/R_w,  F_wsl = f_l - m_w R_w ddθ_wl
# 同理右侧

# ---------------------------
# 4) 用腿的水平式，把 F_bsl/F_bsr 表成 ddθ 与 T
# ---------------------------
# 先把 dds_ll, dds_lr 用运动学替换
dds_ll_lin = eq_ddsll.rhs
dds_lr_lin = eq_ddslr.rhs
eq_legL_h_lin = eq_legL_h.subs({dds_ll: dds_ll_lin}).subs(sol_wl)
eq_legR_h_lin = eq_legR_h.subs({dds_lr: dds_lr_lin}).subs(sol_wr)
print(eq_legR_h_lin)

# 解出 F_bsl, F_bsr
sol_Fb_h = sp.solve([eq_legL_h_lin, eq_legR_h_lin], (F_bsl, F_bsr), dict=True)[0]

# ---------------------------
# 5) (3.13)：把 eq_body_s 代入并用 (2.13) 代替 dds_b，然后整理
# ---------------------------
expr_313 = (eq_body_s.lhs - eq_body_s.rhs) \
    .subs(sol_Fb_h) \
    .subs({dds_b: eq_ddsb.rhs}) \
    .doit()

# 规范化：收集 [ddθ_wl, ddθ_wr, ddθ_ll, ddθ_lr, T_lwl, T_lwr]
expr_313 = sp.expand(expr_313)
expr_313 = sp.collect(expr_313, [ddtheta_wl, ddtheta_wr, ddtheta_ll, ddtheta_lr, T_lwl, T_lwr])

expr_313 = expr_313.xreplace({l_l: l_bl + l_wl, l_r: l_br + l_wr})
eq_313 = sp.Eq(sp.simplify(-expr_313), 0)  

# ---------------------------
# 6) (3.14)：俯仰
sum_Fb_s = sp.simplify(sol_Fb_h[F_bsl] + sol_Fb_h[F_bsr])
sum_Fb_h = (eq_body_h.lhs + m_b*g)

eq_pitch_sub = eq_pitch.subs({
    (F_bsl+F_bsr): sum_Fb_s,      
    (F_bhl+F_bhr): sum_Fb_h
})

eq_pitch_sub = eq_pitch_sub.subs({dds_b: eq_ddsb.rhs, ddh_b: 0})

expr_314 = sp.expand(eq_pitch_sub.lhs - eq_pitch_sub.rhs)
expr_314 = sp.collect(expr_314, [ddtheta_wl, ddtheta_wr, ddtheta_ll, ddtheta_lr,
                                 sp.symbols('ddtheta_b'), theta_b, T_lwl, T_lwr, T_bll, T_blr])
expr_314 = expr_314.xreplace({l_l: l_bl + l_wl, l_r: l_br + l_wr})

eq_314 = sp.Eq(sp.simplify(expr_314), 0)

# ---------------------------
# 7) (3.15)：偏航。先用 (2.11) 线性化的 ddphi，再用轮-力矩关系替换 f_l,f_r
# ---------------------------
expr_yaw_rhs = ((-sol_wl[f_l] + sol_wr[f_r]) * R_l)  # RHS of yaw
expr_yaw_lhs = (I_z * eq_ddphi.rhs)                  # LHS of yaw

expr_315 = sp.expand(expr_yaw_lhs - expr_yaw_rhs)
expr_315 = sp.collect(expr_315, [ddtheta_wl, ddtheta_wr, ddtheta_ll, ddtheta_lr, T_lwl, T_lwr])
expr_315 = expr_315.xreplace({l_l: l_bl + l_wl, l_r: l_br + l_wr})

eq_315 = sp.Eq(sp.simplify(expr_315), 0)
...
# ---------------------------
print("\n=== (3.13) 自动消元/线性化 ===")
sp.pretty_print(eq_313)

print("\n=== (3.14) 自动消元/线性化 ===")
sp.pretty_print(eq_314)

print("\n=== (3.15) 自动消元/线性化 ===")
sp.pretty_print(eq_315)

def coef(expr, sym):
    return sp.simplify(sp.diff(expr, sym))
\end{minted}
...
\begin{aligned}
\frac{1}{2R_w}\Bigl(
-R_w \Bigl[ \ddot{\theta}_{l,l} (l_{bl} m_b + l_{wl} m_b + 2l_{wl} m_l)
+ \ddot{\theta}_{l,r} (l_{br} m_b + l_{wr} m_b + 2l_{wr} m_l) \Bigr] \\
&\qquad + 2T_{lwl} + 2T_{lwr}
- (\ddot{\theta}_{wl} + \ddot{\theta}_{wr}) \Bigl[ 2I_w + R_w^2 (m_b + 2m_l + 2m_w) \Bigr]
\Bigr) ={}& 0
\end{aligned}
\end{equation}
...
\begin{aligned}
\frac{1}{R_w}\Bigl(
R_w \bigl( I_b \ddot{\theta}_b + T_{bll} + T_{blr} + \ddot{\theta}_{l,l} l_c l_{wl} m_l
+ \ddot{\theta}_{l,r} l_c l_{wr} m_l - g l_c m_b \theta_b \bigr) \\
&\qquad - (T_{lwl} + T_{lwr}) l_c
+ (\ddot{\theta}_{wl} + \ddot{\theta}_{wr}) l_c \Bigl[ I_w + R_w^2 (m_l + m_w) \Bigr]
\Bigr) ={}& 0
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\frac{1}{2R_l R_w}\Bigl(
I_z R_w \Bigl[ -\ddot{\theta}_{l,l}(l_{bl} + l_{wl}) + \ddot{\theta}_{l,r}(l_{br} + l_{wr}) \Bigr] \\
&\qquad + 2R_l^2 (T_{lwl} - T_{lwr})
+ (\ddot{\theta}_{wr} - \ddot{\theta}_{wl}) (2I_w R_l^2 + I_z R_w^2)
\Bigr) ={}& 0
\end{aligned}
\end{equation}

这三个方程分别从系统的平动、俯仰和偏航三个自由度出发，揭示了轮足机器人在复杂动力学耦合下的运动规律，为后续的控制器设计提供了理论基础。以下将对每个方程的物理意义进行详细解析。
3.3 纵向平移运动学平衡方程\ref{eq:longitudinal}
本节构建了轮足机器人在纵向（$x$轴方向）的平移加速度与驱动力矩之间的映射关系。根据达朗贝尔原理（D'Alembert's Principle），系统在前进方向上的广义力平衡方程可表述为：$$\sum F_x = M_{eq} \cdot a_x + F_{coupling}$$等效质量与转动惯量折算：方程 (3.13) 中的系数项 $2I_w + R_{w}^{2}(m_b + 2m_l + 2m_w)$ 反映了系统的等效平移惯量。其中不仅包含机体、腿部及轮组的静质量，还通过轮径 $R_w$ 将驱动轮的转动惯量 $I_w$ 折算至线加速度维度。这体现了系统在平移过程中，能量在平动动能与转动动能之间的耦合分布。腿部运动耦合（Dynamic Coupling）：项 $\ddot{\theta}_{l,l}$ 与 $\ddot{\theta}_{l,r}$ 的系数代表了腿部摆动产生的惯性力对底盘平移的影响。当腿部角度发生剧烈变化时，其质心位置的水平加速度会通过关节反作用力直接作用于底盘。驱动输入：$2T_{lw}$ 为系统的主动控制量，代表左右电机在接触点产生的等效牵引力。
3.4 俯仰轴（绕横轴）动力学平衡方程
俯仰方向的稳定性是轮足机器人平衡控制的核心，该方程本质上是一个线性化后的倒立摆模型（Inverted Pendulum Model）。姿态动力学项：$I_b \ddot{\theta}_b$ 描述了机体绕其质心的旋转惯性。不稳定重力矩（Gravity-induced Moment）：$-g l_c m_b \theta_b$ 项具有正反馈特性，代表了重力在机体偏离平衡位置时产生的倾覆力矩。在线性化分析中，该项决定了系统的开环不稳定性，是控制器设计时需要补偿的主要干扰。位移-姿态耦合项：$\ddot{\theta}_{w}$ 项的引入体现了“加速度诱导力矩”效应。当底盘产生水平加速度时，由于机体质心与轮轴存在高度差 $l_c$，会在俯仰方向产生显著的惯性耦合力矩。这也是实现“以位移保平衡”控制策略的物理基础。
3.5 偏航轴（绕竖直轴）动力学平衡方程
偏航（Yaw）方程描述了机器人 heading 角的演化逻辑，主要涉及空间动力学的角动量守恒与差速驱动原理。偏航惯量矩阵：系数 $2I_w R_{l}^{2} + I_z R_{w}^{2}$ 定义了系统绕几何中心转动的总等效惯量。它综合考虑了机体绕 $Z$ 轴的转动惯量 $I_z$ 以及左右轮组在差速转动时的旋转动量。差速驱动逻辑：该方程的核心输入为 $(T_{lw,l} - T_{lw,r})$。通过建立电机力矩差与偏航加速度 $\ddot{\psi}$ 的线性关系，将复杂的地面摩擦力约束简化为由力矩差驱动的二阶系统，简化了偏航控制器的解耦设计。非对称干扰补偿：腿部伸缩或摆动加速度的非对称性（$\ddot{\theta}_{l,r} - \ddot{\theta}_{l,l}$）会导致偏航干扰。方程中对应的系数项为控制算法提供了前馈补偿的理论依据，以确保机器人在高速直线行驶中不发生偏离。
