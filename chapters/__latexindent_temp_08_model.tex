...
下图给出同一模型的另一视角，用于更清楚地展示机体、轮子与腿部的相对位置关系。
...
% 4. 机体 (Base)
\begin{scope}[shift={(BodyCenter)}]
\draw[thick, fill=white, fill opacity=0.8] (-0.8,-1.2,-0.6) -- (1.2,-1.2,-0.6) -- (1.2,1.2,-0.6) -- (-0.8,1.2,-0.6) -- cycle;
\draw[thick] (-0.8,-1.2,-0.6) -- (-0.8,-1.2,0.6);
\draw[thick] (1.2,-1.2,-0.6) -- (1.2,-1.2,0.6);
\draw[thick] (1.2,1.2,-0.6) -- (1.2,1.2,0.6);
\draw[thick] (-0.8,1.2,-0.6) -- (-0.8,1.2,0.6);
...
\begin{tikzpicture}[>=latex, line width=0.8pt]
    ...
    % 腿部中点标记和标签
    \filldraw[fill=white] ($(WheelCenter)!0.5!(BodyCenter)$ African) circle (2pt) node[right=5pt] {$I_{l,l} \quad m_l$};
    % 腿部分段标记 (叉号)
    \node at (LegMid) {$\times$};
    ...
    % 旋转力矩
    \draw[->] ($(WheelCenter) + (0.5, 0.2)$) arc[start angle=20, end angle=-200, radius=0.4] node[above right] {$T_{wl}$};
    ...
    % 旋转力矩
    \draw[->] ($(BodyCenter) + (-0.3, 0.4)$) arc[start angle=120, end angle=380, radius=0.4] node[right=2pt] {$T_{bl}$};
    ...
    \begin{scope}[shift={(3.5, 0.5)}, rotate=\angle]
        ...
        \draw[<->] (0, \Lw) -- (0, \Lw+\Lb) node[midway, right=3pt, rotate=\angle-90] {$l_{b,l}$};
